...
// @grant        none
// @updateURL        https://github.com/personwritep/Ameba_Search_Tools/raw/main/Ameba_Search_Tools.user.js
// @downloadURL        https://github.com/personwritep/Ameba_Search_Tools/raw/main/Ameba_Search_Tools.user.js
// ==/UserScript==
...
if(location.hostname=='search.ameba.jp'){
    ...
    function page_select(){
        if(document.querySelector('.PcResultPagination')){ // 検索ページャーのある画面でのみ動作
            ...
            function list_listen(){ // 記事リストのクリック操作の変更
                let items=get_items();
                for(let k=0; k<items.length; k++){
                    items[k].addEventListener('click', function(event){
                        event.preventDefault();
                        event.stopImmediatePropagation();
                        let pass=items[k].querySelector('a').getAttribute('href');
